...
def softmax(x):
    ...
    ### YOUR CODE HERE
    if len(x.shape) == 1:
        x -= tf.reduce_max(x) # numerical stability
        x = tf.exp(x)
        out = x / tf.reduce_sum(x) 
    else:
        x -= tf.reshape(tf.reduce_max(x, axis=1), [-1, 1]) # numerical stability
        x = tf.exp(x)
        #out = x / tf.reduce_sum(x, axis=1)
        out = x / tf.reshape(tf.reduce_sum(x, axis=1), [-1, 1])
    ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
    ...
    idx = tf.stack((range(y_true.shape[0]), y_true), axis=1)
    scores = tf.gather_nd(yhat, idx)
    out = -tf.reduce_sum(tf.log(scores))
    ### END YOUR CODE
