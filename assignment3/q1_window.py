...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    windowed_data = []
    n_word_features = len(data[0])
    n_window_features = n_word_features * (2 * window_size + 1)
    for sentence, labels in data:
    ### YOUR CODE HERE (5-20 lines)
        flatten_words = [start for i in range(window_size)] + sentence + [end for i in range(window_size)]
        flatten_words = [feature for words in flatten_words for feature in words]
        for i in range(len(labels)):
            windowed_data.append((flatten_words[n_word_features * i: n_word_features * i + n_window_features], labels[i]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embedded = tf.get_variable('embedded', initializer=self.pretrained_embeddings)
    ...
    def add_prediction_op(self):
        ...
        b2 = tf.get_variable('b2', self.config.n_classes, initializer=tf.zeros_initializer, dtype=tf.float32)
