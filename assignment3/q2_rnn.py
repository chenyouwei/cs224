...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if padding >= 0:
            ret.append([sentence + [zero_vector] * padding, labels + [zero_label] * padding, [True] * len(sentence) + [False] * padding])
        else:
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        self.input_placeholder = tf.placeholder(tf.int32, shape=(None, self.config.max_length, self.config.n_features), name='input')
        self.labels_placeholder = tf.placeholder(tf.int32, shape=(None, self.config.max_length), name='label')
    ...
    def add_loss_op(self, preds):
        ...
        losses = tf.boolean_mask(losses, self.mask_placeholder)
